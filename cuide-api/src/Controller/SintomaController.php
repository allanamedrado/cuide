...
class SintomaController extends AbstractController
{
    ...
    /**
     * @Route("/{sintomaId}/edit", name="sintoma_edit", methods={"GET","POST"})
     */
    ...
    /**
     * @Route("/{sintomaId}", name="sintoma_delete", methods={"POST"})
     */
    public function delete(Request $request, Sintoma $sintoma): Response
    {
        if ($this->isCsrfTokenValid('delete'.$sintoma->getSintomaId(), $request->request->get('_token'))) {
            $entityManager = $this->getDoctrine()->getManager();
